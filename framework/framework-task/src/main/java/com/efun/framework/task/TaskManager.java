...
import com.efun.framework.task.entity.builder.PopTaskBuilder;
...
import com.efun.framework.task.util.JsonUtil;
...
public class TaskManager {
    ...
    public PopTask popPendingTask(String handler, Date date) {
        ...
        if (CollectionUtils.isEmpty(taskKeys)) {
            return PopTaskBuilder.build(null, PopTaskResult.failSleep);
        }
        if (taskKeys.size() > 1) {
        ...
        if (StringUtils.isBlank(taskKey)) {
            ...
            return PopTaskBuilder.build(null, PopTaskResult.tryNext);
        }
